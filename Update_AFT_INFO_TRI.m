function AFT_stack = Update_AFT_INFO_TRI(AFT_stack, node1_base, node2_base, node_select, nCells_AFT , xCoord_AFT, yCoord_AFT)
global cellNodeTopo;
...
if( flag1 == 1 )
    AFT_stack(1,3) = nCells_AFT;
else
    AFT_stack(1,4) = nCells_AFT;
end
...
[direction, row] = FrontExist(node_select,node1_base, AFT_stack);  %判断阵面（node_select，node1_base）是否已经存在及其在AFT_stack_sorted中的方向
if( flag2 == 1 )  %如果为左单元
    if( row ~= -1 )   %如果已经存在
        if(direction == 1) %如果AFT_stack_sorted中存储方向为（node_select，node1_base）
            AFT_stack(row, 3) = nCells_AFT;  %则更新到左单元
        elseif(direction == -1)
            AFT_stack(row, 4) = nCells_AFT;  %否则更新到右单元
        end
    else    %如果不存在，则按反的逻辑新增阵面，即阵面方向反过来（node1_base, node_select），nCells_AFT更新到右单元，左单元不更新
        AFT_stack(end+1,:) = [node1_base, node_select, -1, nCells_AFT, dist1,  size(AFT_stack,1)+1];  
    end
else   %如果为右单元
    if( row ~= -1 )  %如果已经存在
        if(direction == 1)  %如果AFT_stack_sorted中存储方向为（node_select，node1_base）
            AFT_stack(row, 4) = nCells_AFT; %更新到右单元
        elseif(direction == -1)
            AFT_stack(row, 3) = nCells_AFT; %否则更新到左单元
        end
    else  %如果不存在，则按反的逻辑新增阵面，即阵面方向反过来（node1_base, node_select），nCells_AFT更新到左单元，右单元不更新
        AFT_stack(end+1,:) = [node1_base, node_select, nCells_AFT, -1, dist1,  size(AFT_stack,1)+1];  
    end
end 

flag3 = IsLeftCell(node2_base, node_select, node1_base, xCoord_AFT, yCoord_AFT);
[direction, row] = FrontExist(node2_base,node_select, AFT_stack); 
if( flag3 == 1 ) %如果为左单元
    if( row ~= -1 )%如果已经存在
        ...
        AFT_stack(end+1,:) = [node_select, node2_base, -1, nCells_AFT, dist2,  size(AFT_stack,1)+1];  
    end
else %如果为右单元
    if( row ~= -1 ) %如果已经存在
        if(direction == 1) %如果AFT_stack_sorted中存储方向为（node2_base, node_select）
        ...
        AFT_stack(end+1,:) = [node_select, node2_base, nCells_AFT, -1, dist2,  size(AFT_stack,1)+1];              
    end
end      