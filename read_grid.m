...
% AFT_stack = zeros(nBoundaryFaces,3);
% for i = 1:nBoundaryFaces
%     AFT_stack(i,1) = node1(i,1);
%     AFT_stack(i,2) = node2(i,1);
%     AFT_stack(i,3) = DISTANCE(AFT_stack(i,1), AFT_stack(i,2), xCoord, yCoord);
% end

nodeList = AFT_stack(:,1:2);
npoints = max( max(nodeList)-min(nodeList)+1 ); %边界点的个数，或者，初始阵面点数
Coord = Coord(1:npoints,:);                    %初始阵面点坐标